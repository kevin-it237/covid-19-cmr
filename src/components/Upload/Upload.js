...
export default class Upload extends Component {
    ...
    render() {
        ...
        const { label, oldUrl, type, disabled } = this.props;
        let accept = type === "image" ? "image/png,image/jpeg,image/jpg" : (type === "document" ? ".docx, .pptx, .pdf" : "video/*");

    return ( 
        <Fragment>
            <button type="button" className="button" onClick={()=>this.inputElement.click()}>{label} </button>
            <input style={{visibility:'hidden'}} accept={accept} disabled={disabled} onChange={(event) => this.preview(event)} ref={input => this.inputElement = input} type="file" />
            <div className="ibox">
                <div className="ibox-content product-box active">
                    <div className={!url&&!oldUrl?'product-imitation':''}>
                    {!(url||oldUrl)? '':'' }
                     {(url||oldUrl)&&type==="image"?<img src={url||oldUrl} accept={accept} alt="" />:''}
